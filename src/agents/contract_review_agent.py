...
class ContractReviewAgent(BaseAgent):
    ...
    def _build_system_prompt(self, **kwargs: Any) -> str:
        ...
        prompt_parts = [
            ...
            "3. 按顺序审核每一条款，分析其法律风险和对甲方的影响",
            "4. 对每个发现的问题，立即使用 manage_issues 的 add 操作记录",
            "5. 审核完所有条款后，输出该章节的审核总结",
            "",
            "## 可用工具",
            "- manage_issues: 管理问题点（add 添加、update 更新、delete 删除、list 列出、get_current_section 获取当前章节等）；可用 update/delete 修正已有问题点。",
